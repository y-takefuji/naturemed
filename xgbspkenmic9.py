...
def select_top_features(data, target, method_name, n=9):  # Changed default from 10 to 9
    importance = {}
...
# 3. Top 9 feature selection from full feature set (12 features)
xgb_top9, xgb_importance = select_top_features(X, y, method_name='xgboost', n=9)
spearman_top9, spearman_importance = select_top_features(X, y, method_name='spearman', n=9)
kendall_top9, kendall_importance = select_top_features(X, y, method_name='kendall', n=9)
mic_top9, mic_importance = select_top_features(X, y, method_name='mic', n=9)

# 4. Create reduced datasets with top 9 features from each method
X_xgb_reduced = X[xgb_top9]
X_spearman_reduced = X[spearman_top9]
X_kendall_reduced = X[kendall_top9]
X_mic_reduced = X[mic_top9]

# 5. Print the top 9 features selected by each method
print("\n========== TOP 9 FEATURES SELECTED FROM FULL DATASET ==========")
print(f"XGBoost:  {xgb_top9}")
print(f"Spearman: {spearman_top9}")
print(f"Kendall:  {kendall_top9}")
print(f"MIC:      {mic_top9}")
...
print("\nFrom reduced dataset (9 features):")  # Updated comment
...
# 9b. Cross-validation with reduced feature sets (9 features each)
print("\n========== CROSS-VALIDATION WITH REDUCED FEATURE SETS (9 FEATURES) ==========")  # Updated comment
datasets = {
...
for feat, imp in print_sorted_importance(xgb_importance, 9):  # Changed from 10 to 9
    print(f"  {feat:20s} {imp:.3f}")

print("\nSpearman feature importances:")
for feat, imp in print_sorted_importance(spearman_importance, 9):  # Changed from 10 to 9
    print(f"  {feat:20s} {imp:.3f}")

print("\nKendall feature importances:")
for feat, imp in print_sorted_importance(kendall_importance, 9):  # Changed from 10 to 9
    print(f"  {feat:20s} {imp:.3f}")

print("\nMIC feature importances:")
for feat, imp in print_sorted_importance(mic_importance, 9):  # Changed from 10 to 9
    print(f"  {feat:20s} {imp:.3f}")
