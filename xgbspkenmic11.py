...
def select_top_features(data, target, method_name, n=11):
    importance = {}
...
# 3. Top 11 feature selection from full feature set (12 features)
xgb_top11, xgb_importance = select_top_features(X, y, method_name='xgboost', n=11)
spearman_top11, spearman_importance = select_top_features(X, y, method_name='spearman', n=11)
kendall_top11, kendall_importance = select_top_features(X, y, method_name='kendall', n=11)
mic_top11, mic_importance = select_top_features(X, y, method_name='mic', n=11)

# 4. Create reduced datasets with top 11 features from each method
X_xgb_reduced = X[xgb_top11]
X_spearman_reduced = X[spearman_top11]
X_kendall_reduced = X[kendall_top11]
X_mic_reduced = X[mic_top11]

# 5. Print the top 11 features selected by each method
print("\n========== TOP 11 FEATURES SELECTED FROM FULL DATASET ==========")
print(f"XGBoost:  {xgb_top11}")
print(f"Spearman: {spearman_top11}")
print(f"Kendall:  {kendall_top11}")
print(f"MIC:      {mic_top11}")
...
print("\nFrom reduced dataset (11 features):")
...
# 9b. Cross-validation with reduced feature sets (11 features each)
print("\n========== CROSS-VALIDATION WITH REDUCED FEATURE SETS (11 FEATURES) ==========")
datasets = {
...
for feat, imp in print_sorted_importance(xgb_importance, 11):
    print(f"  {feat:20s} {imp:.3f}")

print("\nSpearman feature importances:")
for feat, imp in print_sorted_importance(spearman_importance, 11):
    print(f"  {feat:20s} {imp:.3f}")

print("\nKendall feature importances:")
for feat, imp in print_sorted_importance(kendall_importance, 11):
    print(f"  {feat:20s} {imp:.3f}")

print("\nMIC feature importances:")
for feat, imp in print_sorted_importance(mic_importance, 11):
    print(f"  {feat:20s} {imp:.3f}")
